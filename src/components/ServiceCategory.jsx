...
export default function ServiceCategory({ service, index }) {
  ...
  return (
    <div className="group border-b border-line-inv">
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        aria-expanded={open}
      ...
      >
        <div className="flex items-start gap-6 md:gap-10">
          <span className="mt-2 text-[12px] uppercase tracking-widest2 text-muted-inv">
            {number}
          </span>
      ...
      >
        <div className="min-h-0">
          <div className="flex flex-col gap-8 pb-12 pl-0 md:flex-row md:justify-between md:gap-16 md:pb-16 md:pl-[4.5rem]">
            <p className="max-w-sm text-[16px] leading-relaxed text-muted-inv md:text-[18px]">
              {service.description}
            </p>
