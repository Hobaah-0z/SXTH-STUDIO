    ...
    <article ref={ref}>
      ...
      <Link to={`/work/${project.slug}`} className="group block">
        <motion.div style={{ scale, y }} className={`relative overflow-hidden bg-white/5 ${index % 2 ? 'md:ml-[12%]' : 'md:mr-[12%]'}`}>
          <div className="aspect-[16/10] md:aspect-[16/9]"><Parallax amount={7} className="h-full w-full"><FeaturedProjectMedia image={project.image} video={project.video} playOnView alt={`${project.name} — ${project.category}`} label={project.name} className="h-full w-full" /></Parallax></div>
